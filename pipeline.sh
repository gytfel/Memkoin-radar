...
# Прогон идёт десятки минут. По ssh это верный способ потерять работу на
# середине: рвётся соединение — умирает и скрипт. tmux переживает обрыв,
# сеанс потом подхватывается заново.
if [ -n "${SSH_CONNECTION:-}" ] && [ -z "${TMUX:-}" ] && [ -z "${STY:-}" ]; then
    cat <<'WARN'

  ⚠️  Запущено по ssh без tmux.

  Прогон занимает десятки минут, и обрыв связи оборвёт его на середине.
  Надёжнее так:

      tmux new -s radar
      bash pipeline.sh

  Если связь отвалится — вернуться в тот же сеанс: tmux attach -t radar
  Выйти, оставив работать: Ctrl+B, затем D

  Продолжаю через 10 секунд. Прервать: Ctrl+C
...
# Не "|| echo 0": grep при нуле совпадений печатает 0 И возвращает код 1,
# поэтому echo дописывал вторую строку. В переменной оказывалось "0\n0",
# а тест падал с "integer expression expected" — и проверка молча не работала.
CANDS=$(grep -c '^[^#[:space:]]' candidates.txt 2>/dev/null) || CANDS=0
if [ "$CANDS" -eq 0 ]; then
